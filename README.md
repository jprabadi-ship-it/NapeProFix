...
- 電池残量をメニューバーアイコンの右に表示（Bluetooth 接続時のみ。下記「制約」参照）
- ログイン時に起動
...
あわせて Bluetooth の使用許可も求められます。電池残量の読み取りにだけ使うもので、拒否しても他の機能には影響しません（残量が表示されないだけです）。
...
- 電池残量は **Bluetooth 接続時にしか取れません**。BLE の Battery Service（0x180F）から読んでいます。2.4GHz ドングルやケーブルでは USB HID として繋がり、Battery System の UsagePage（0x85）を公開していないため、読む場所がありません。VIA チャンネル（0xFF60）にも電池はなく、macOS が IORegistry に転記することもありません（いずれも実測）。ドングル接続中は何も表示しません
